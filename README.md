...
In order to run the analysis, import the two cvs files (pay attention, the columns are separated by ; ) and run the desired query.
...
What is the average number of yellow cards per role? And which players are significantly above their role average?
